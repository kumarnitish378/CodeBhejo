...
def download_qr_code(username):
    ...
    file_location = create_image_with_text(qr_code_image_path, shop_name[0:25], f"C-ID {username}")
...
@app.route('/newfile/<username>', methods=['GET', 'POST'])
def newfile(username):
    BOOKS = ["C", "C++", "Python", "Java", "JavaScript", "HTML", "ASM"] # get_all_book()
    MESSAGE = f"Roll Number: {username}"
    if request.method == 'POST':
        # print(request.form)
        NAME = request.form['name']
        CATEGORY = request.form['category']
        CONTENT = request.form['editor1']
        Book_ID = 1 # get_book_id(NAME)
        if Book_ID == None:
            MESSAGE = "Book not found"
            return render_template('add_post.html', message=MESSAGE, books=BOOKS)
        MESSAGE = "Post Added Successfully"
        TS  =  time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime())
        # insert_chapter(NAME, CATEGORY, CONTENT, TS, Book_ID)
        print(remove_html_and_entities(CONTENT))
        with open("File.py", "w") as f:
            f.write(remove_html_and_entities(CONTENT))
        MESSAGE = "Successfully added"
        return render_template('texteditor.html', categories=BOOKS, MESSAGE=MESSAGE)
    return render_template('texteditor.html', categories=BOOKS, MESSAGE=MESSAGE)
